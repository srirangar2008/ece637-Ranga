...
int main (int argc, char **argv) 
{
  ...
  //The code for adding the filter to filter red, green and blue components
  for(int i = 0; i < input_img.height; i++)
  {
    for(j = 1; j < input_img.width - 1; j++)
    {
      color_img.color[0][i][j] = (input_img.color[0][i][j-1] + input_img.color[0][i][j] + input_img.color[0][i][j+1])/3.0;
      color_img.color[1][i][j] = (input_img.color[1][i][j-1] + input_img.color[1][i][j] + input_img.color[1][i][j+1])/3.0;
      color_img.color[2][i][j] = (input_img.color[2][i][j-1] + input_img.color[2][i][j] + input_img.color[2][i][j+1])/3.0;
      color_img.color[0][i][j] += 32*normal();
      color_img.color[1][i][j] += 32*normal();
      color_img.color[2][i][j] += 32*normal();
    }
  }
  ...
  if ( ( fp = fopen ( "color_2.tif", "wb" ) ) == NULL ) {
